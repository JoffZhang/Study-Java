...
public class LFUCache2 {
	...
	public static void main(String[] args) {
		LFUCache2 cache = new LFUCache2(3 /* capacity (缓存容量) */);
	...
	void debug() {
		System.out.println(freqNodeDLLMap.toString());
		System.out.println(tos(keyNodeMap));
		System.out.println("========");
	}
	String tosLM(Map<Integer, NodeDLinkedList> ma) {
	...
	String tos(Map<Integer, LFUCache2.Node> ma) {
		StringBuilder builder = new StringBuilder();
		for (int i : ma.keySet()) {
			builder.append(i + ":" + ma.get(i) + "  ");
		}
	...
	private class NodeDLinkedList{
		...
		public String toString() {
			StringBuilder builder = new StringBuilder();
			if(head == tail){
				builder.append(String.format("(%d:%d,%d)->", head.key, head.value, head.frequency));
			}else{
				for (LFUCache2.Node i = head.next; i != tail; i = i.next) {
